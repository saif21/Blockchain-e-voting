...
class Blockchain:
    ...
    def __init__(self):
        ...
        if len(self.get_chain()) < 1:
            ...
            vote.minevote3(1, '0', 1, str(datetime.datetime.now()),
    ...
    def get_chain(self):
        chains = vote.loadblock3()  # need change to loadblock1-5
        self.chain = []
        for i in chains:
            d = {}
            d['index'] = i['id']
    ...
    def replace_chain(self):
        ...
        if len(longest_chain) > len(current_chain) and self.is_chain_valid(longest_chain):
            ...
            replace.update_block_db3(longest_chain)  # need to change to db1-5
            return True
        return False
    ...
    if request.method == 'POST':
        if blockchain.orphan_vote:
            if blockchain.is_chain_valid(blockchain.get_chain()):
                ...
                vote.minevote3(block['index'], block['previous_hash'],
    ...
    if request.method == 'POST':
        if blockchain.is_chain_valid(blockchain.get_chain()):
            ...
            if len(blockchain.votes) > 2:
                ...
                vote.minevote3(block['previous_hash'],
                               block['proof'], block['timestamp'], json.dumps(block['votes']))
